...
// Har element pe decide karo:

// Kya mujhe naya subarray shuru karna chahiye (arr[i])

// Ya phir pehle wale subarray ka sum me add kar du (currMax + arr[i])

// Agar currMax > globalMax ho jaye → update globalMax and endIndex

// Jab end mil gaya, to globalMax se left jaake start index trace karo
...
function maxSubarraySum(arr) {
  ...
  console.log("Maximum Sum:", globalMax);
  return { subarray, sum: globalMax };
}

// ✅ Test Cases
maxSubarraySum([-2, -3, 4, -1, -2, 1, 5, -3]);  // Output: [4, -1, -2, 1, 5], Sum = 7
maxSubarraySum([-2, -5, 6, -2, -3, 1, 5, -6]);  // Output: [6, -2, -3, 1, 5], Sum = 7
maxSubarraySum([-1, -2, -3, -4]);               // Output: [-1], Sum = -1
