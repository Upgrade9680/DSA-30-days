...
// Jab bhi pattern ke beech mein mismatch milta hai, hum already matched portion ka longest proper prefix which is also a suffix use karke pata lagate hain ki next kaha se comparison continue karna hai — bina puri pattern ko dobara match kiye.

// 🔧 2 Steps in KMP:
// LPS Array banate hain (Longest Prefix Suffix)

// Pattern ko text ke andar match karte hain using that LPS array.
...
function KMPSearch(pat, txt) {
  ...
  computeLPSArray(pat, M, lps); // Step 1: Preprocessing
  ...
  while (i < N) {
    ...
    if (j === M) {
      ...
      if (j !== 0) {
        ...
        i++; // No match, move to next character
      }
    }
  }
}

KMPSearch("AABA", "AABAACAADAABAABA");
